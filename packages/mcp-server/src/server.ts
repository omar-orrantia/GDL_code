...
export async function initMcpServer(params: {
  ...
  let _client: StrezlessMusickNexusMetadata | undefined;
  let _clientError: Error | undefined;
  let _logLevel: 'debug' | 'info' | 'warn' | 'error' | 'off' | undefined;

  const getClient = (): StrezlessMusickNexusMetadata => {
    if (_clientError) throw _clientError;
    if (!_client) {
      try {
        _client = new StrezlessMusickNexusMetadata({
          logger,
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    let client: StrezlessMusickNexusMetadata;
    try {
      client = getClient();
    } catch (error) {
      return {
