...
export { ClientOptions } from 'strezless-musick-nexus-metadata';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ??
      'https://api.stainless.com/api/ai/instructions/strezless-musick-nexus-metadata',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the strezless-musick-nexus-metadata MCP server. You will use Code Mode to help the user perform
...
export const newMcpServer = async () =>
  new McpServer(
    {
      name: 'strezless_musick_nexus_metadata_api',
      version: '0.0.1',
    },
    {
...
export async function initMcpServer(params: {
  ...
  let client = new StrezlessMusickNexusMetadata({
    logger,
