...
import { StrezlessMusickNexusMetadata, ClientOptions } from 'strezless-musick-nexus-metadata';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: StrezlessMusickNexusMetadata)`
    : `const run: (${functionSource.client}: StrezlessMusickNexusMetadata) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
