...
import { APIPromise } from 'strezless-musick-nexus-metadata/core/api-promise';

import util from 'node:util';
import StrezlessMusickNexusMetadata from 'strezless-musick-nexus-metadata';
import { APIUserAbortError } from 'strezless-musick-nexus-metadata';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['STREZLESS_MUSICK_NEXUS_METADATA_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: StrezlessMusickNexusMetadata) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new StrezlessMusickNexusMetadata({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['STREZLESS_MUSICK_NEXUS_METADATA_LOG'] = 'debug';
      const client = new StrezlessMusickNexusMetadata({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['STREZLESS_MUSICK_NEXUS_METADATA_LOG'] = 'not a log level';
      const client = new StrezlessMusickNexusMetadata({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'STREZLESS_MUSICK_NEXUS_METADATA_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['STREZLESS_MUSICK_NEXUS_METADATA_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['STREZLESS_MUSICK_NEXUS_METADATA_BASE_URL'] = 'https://example.com/from_env';
      const client = new StrezlessMusickNexusMetadata({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['STREZLESS_MUSICK_NEXUS_METADATA_BASE_URL'] = ''; // empty
      const client = new StrezlessMusickNexusMetadata({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });

    test('blank env variable', () => {
      process.env['STREZLESS_MUSICK_NEXUS_METADATA_BASE_URL'] = '  '; // blank
      const client = new StrezlessMusickNexusMetadata({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['STREZLESS_MUSICK_NEXUS_METADATA_BASE_URL'] = 'http://localhost:5000/env';
      const client = new StrezlessMusickNexusMetadata({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new StrezlessMusickNexusMetadata({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new StrezlessMusickNexusMetadata({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['STREZLESS_MUSICK_NEXUS_METADATA_API_KEY'] = 'My API Key';
    const client = new StrezlessMusickNexusMetadata();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['STREZLESS_MUSICK_NEXUS_METADATA_API_KEY'] = 'another My API Key';
    const client = new StrezlessMusickNexusMetadata({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new StrezlessMusickNexusMetadata({ apiKey: 'My API Key', fetch: testFetch });
